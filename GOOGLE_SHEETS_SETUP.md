...
같은 이유로 글자색은 네이비(주요) · `#333333`(보조) 두 단계만 씁니다. 보조색을 더 연하게 두면 작은 글자가 흐려져 잘 안 보입니다. 편명의 골드와 푸터의 베이지는 예외입니다.
색이 여러 가지면 반전 뒤 위계가 흐트러지고, `#9CA3AF` 같은 중간 회색은 뒤집혀도 중간 회색이라 어두운 배경에서 묻힙니다.
